...
export default async function DoctorPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f4f6f5' }}>
        <div style={{ textAlign: 'center' }}>
          <h1 style={{ fontFamily: 'Lora, serif', fontSize: '2rem', color: '#0e3a3f', marginBottom: '12px' }}>Doctor not found</h1>
  ...
  return (
    <div style={{ background: '#f4f6f5', minHeight: '100vh' }}>
      ...
      <div style={{ paddingTop: '70px', background: 'linear-gradient(135deg, #0e3a3f 0%, #28393c 100%)' }}>
      ...
      <div style={{ padding: '80px 8%', display: 'grid', gridTemplateColumns: '1fr 360px', gap: '60px' }}>
        <div>
          <div style={{ marginBottom: '48px' }}>
            <h2 style={{ fontFamily: 'Lora, serif', fontSize: '1.8rem', color: '#0e3a3f', marginBottom: '20px' }}>About</h2>
            <p style={{ fontSize: '1rem', color: '#7a8a8f', lineHeight: 1.8 }}>{doc.about}</p>
          </div>
          <div style={{ marginBottom: '48px' }}>
            <h2 style={{ fontFamily: 'Lora, serif', fontSize: '1.8rem', color: '#0e3a3f', marginBottom: '20px' }}>Education & Training</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {doc.education.map((edu) => (
                <div key={edu} style={{ display: 'flex', alignItems: 'center', gap: '12px', background: 'white', padding: '16px 20px', borderRadius: '12px', border: '1px solid #dde7e5' }}>
                  <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#db6a47', flexShrink: 0 }} />
                  <span style={{ fontSize: '0.88rem', color: '#28393c' }}>{edu}</span>
                </div>
              ))}
            </div>
          </div>
          <div style={{ marginBottom: '48px' }}>
            <h2 style={{ fontFamily: 'Lora, serif', fontSize: '1.8rem', color: '#0e3a3f', marginBottom: '20px' }}>Services</h2>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              {doc.services.map((s) => (
                <div key={s} style={{ display: 'flex', alignItems: 'center', gap: '12px', background: 'white', padding: '14px 18px', borderRadius: '12px', border: '1px solid #dde7e5' }}>
                  <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#db6a47', flexShrink: 0 }} />
                  <span style={{ fontSize: '0.85rem', color: '#28393c', fontWeight: 500 }}>{s}</span>
                </div>
              ))}
            </div>
          </div>
          <div>
            <h2 style={{ fontFamily: 'Lora, serif', fontSize: '1.8rem', color: '#0e3a3f', marginBottom: '20px' }}>Languages</h2>
            <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
              {doc.languages.map((lang) => (
                <div key={lang} style={{ background: 'white', border: '1px solid #dde7e5', padding: '10px 20px', borderRadius: '100px', fontSize: '0.85rem', color: '#28393c', fontWeight: 500 }}>
                  {lang}
                </div>
              ))}
            </div>
        ...
        <div style={{ position: 'sticky', top: '90px' }}>
          <div style={{ background: 'white', borderRadius: '20px', padding: '28px', border: '1px solid #dde7e5', marginBottom: '20px' }}>
            <h3 style={{ fontFamily: 'Lora, serif', fontSize: '1.1rem', color: '#0e3a3f', marginBottom: '16px' }}>Availability</h3>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '20px' }}>
              {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'].map((day) => (
                <div key={day} style={{ padding: '6px 14px', borderRadius: '100px', fontSize: '0.78rem', fontWeight: 600, background: doc.availability.includes(day) ? '#dcebe9' : '#f8f8f8', color: doc.availability.includes(day) ? '#db6a47' : '#b0b0b0', border: `1px solid ${doc.availability.includes(day) ? '#e8c4b0' : '#e8e8e8'}` }}>
                  {day.slice(0, 3)}
                </div>
              ))}
            </div>
            <a href="/#appointment" style={{ display: 'block', background: '#db6a47', color: 'white', padding: '14px', borderRadius: '100px', textDecoration: 'none', fontWeight: 600, fontSize: '0.9rem', textAlign: 'center' }}>
              Book Appointment
            </a>
          </div>
          <div style={{ background: 'white', borderRadius: '20px', padding: '28px', border: '1px solid #dde7e5' }}>
            <h3 style={{ fontFamily: 'Lora, serif', fontSize: '1.1rem', color: '#0e3a3f', marginBottom: '16px' }}>Contact</h3>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '0.88rem', fontWeight: 600, color: '#0e3a3f' }}>+1 (800) 382-7227</div>
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '0.88rem', fontWeight: 600, color: '#0e3a3f' }}>{doc.specialty}</div>
              </div>
            </div>
          </div>
        </div>
